...
1. setTimeout() запускает функцию с задержкой, а setInterval() постоянно запускает функции через указанный промежуток времени.
2. Сработает немедленно. Минимальная задержка таймера браузера от 0 до 4мс. Зависит от браузера.
3. Чтобы быть уверенным, что другой код не выполниться перед предыдущим.
